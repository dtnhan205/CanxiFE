...
const CreateProduct = () => {
  ...
  const validateForm = () => {
    ...
    if (imagePreviews.length === 0 || imagePreviews.length > 4) newErrors.images = 'Số lượng hình ảnh phải từ 1 đến 4';
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.discountPrice) data.append('discountedPrice', parseInt(formData.discountPrice)); // Chỉ gửi nếu có giá trị
    ...
    try {
      ...
      setErrors({ ...errors, apiError: err.message || 'Đã xảy ra lỗi khi tạo sản phẩm.' });
    } finally {
      setSubmitLoading(false);
    }
  };
      ...
      <form onSubmit={handleSubmit} className="product-form">
        ...
        <div className="form-actions">
          ...
          <button type="submit" className="btn-formadmin btn-primary" disabled={submitLoading}>
            {submitLoading ? <><i className="fas fa-spinner fa-spin"></i> Đang tạo...</> : <><i className="fas fa-plus"></i> Tạo sản phẩm</>}
          </button>
        </div>
      </form>
    </div>
